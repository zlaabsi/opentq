...
- Regenerated the cleanup manifest after publication work; no deletion was performed in that pass.

## Cleanup Decision

Earlier cleanup deleted:
...
The current non-destructive manifest is generated by `scripts/build_qwen36_cleanup_manifest.py`. It preserves BF16, Packed, Metal, and raw/native artifacts, marks public GGUF/reproducibility staging as candidates only after HF re-verification, and leaves HF caches as the only delete-if-disk-pressure class. The decision is documented in `docs/qwen36-disk-cleanup-arbitrage.md`.
