...
OUTPUT_ROOT="${1:-artifacts/qwen3.6-27b-representative-benchmarks}"
STAMP="${RUN_STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
RUN_ROOT="$OUTPUT_ROOT/$STAMP"
SESSION_NAME="${SESSION_NAME:-opentq_qwen36_representative}"
mkdir -p "$RUN_ROOT"

RUN_LOG="$RUN_ROOT/representative.log"
SESSION_FILE="$RUN_ROOT/screen.session"
SCREEN_PID_FILE="$RUN_ROOT/screen.pid"

if ! command -v screen >/dev/null 2>&1; then
  echo "screen is required for detached representative benchmark runs"
  exit 1
fi
...
screen -dmS "$SESSION_NAME" bash -lc "cd '$ROOT_DIR' && RUN_ROOT='$RUN_ROOT' exec caffeinate -dimsu ./scripts/run_qwen36_representative_benchmarks.sh > '$RUN_LOG' 2>&1"
